...
package net.mcreator.galaxycraft.recipe.brewing;
...
@EventBusSubscriber
public class WampaSlayerPotionRecipe2BrewingRecipe implements IBrewingRecipe {
	@SubscribeEvent
	public static void init(RegisterBrewingRecipesEvent event) {
		event.getBuilder().addRecipe(new WampaSlayerPotionRecipe2BrewingRecipe());
	}
	...
	public boolean isIngredient(ItemStack ingredient) {
		return Ingredient.of(new ItemStack(GalaxycraftModItems.WAMPA_FUR.get())).test(ingredient);
	}

	@Override
	public ItemStack getOutput(ItemStack input, ItemStack ingredient) {
		if (isInput(input) && isIngredient(ingredient)) {
			return PotionContents.createItemStack(input.getItem(), GalaxycraftModPotions.WAMPA_SLAYER);
		}
