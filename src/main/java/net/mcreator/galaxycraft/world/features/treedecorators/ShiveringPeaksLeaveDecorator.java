...
public class ShiveringPeaksLeaveDecorator extends LeaveVineDecorator {
	public static MapCodec<ShiveringPeaksLeaveDecorator> CODEC = MapCodec.unit(ShiveringPeaksLeaveDecorator::new);
	public static TreeDecoratorType<?> DECORATOR_TYPE = new TreeDecoratorType<>(CODEC);

	@SubscribeEvent
	public static void registerTreeDecorator(RegisterEvent event) {
		event.register(Registries.TREE_DECORATOR_TYPE, ResourceLocation.parse("galaxycraft:shivering_peaks_tree_leave_decorator"), () -> DECORATOR_TYPE);
	}

	public ShiveringPeaksLeaveDecorator() {
		super(0.25f);
	}
	...
	private static void addVine(BlockPos pos, Direction direction, TreeDecorator.Context context) {
		context.setBlock(pos, Blocks.ICE.defaultBlockState());
		int i = 4;
		for (BlockPos blockpos = pos.below(); context.isAir(blockpos) && i > 0; --i) {
			context.setBlock(blockpos, oriented(Blocks.ICE.defaultBlockState(), direction));
